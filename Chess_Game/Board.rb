...
class Board 
  ...
  def move_piece(color, start_pos, end_pos)
    raise EmptyPlaceError if self[start_pos] == @sentinel || !self.valid_pos?(start_pos)
    raise "error" if !self.valid_pos?(end_pos)
  ...
  def starting_board
    ...
    @rows[1] = @rows[1].map.with_index {|ele, idx| ele = Pawn.new(:white, self, [1, idx])}
    @rows[6] = @rows[6].map.with_index {|ele, idx| ele = Pawn.new(:black, self, [6, idx])}
